...
The project was built around these important requirements from the assignment:

- Use ASP.NET Core with Blazor for the UI
- Use Identity for authentication and user management
- Use a separate API layer for UI communication
- Use EF Core with SQL Server LocalDB
- Model categories, subcategories, questions, answer options, and user results
- Seed the database with quiz content
- Ensure a default user exists on startup with username `user` and password `Password1234!`
- Restrict most pages so only the welcome page, registration, and login are public
- Save every answer result to the database
- Unlock the next subcategory only after the user reaches at least 80% correct in the current one
- Support the AI-based extension for extra functionality
...
- `GET /api/quiz/categories?userId={userId}`
- `GET /api/quiz/subcategories?categoryId={categoryId}&userId={userId}`
- `GET /api/quiz/questions?subCategoryId={subCategoryId}&userId={userId}`
- `POST /api/quiz/submit-answer?userId={userId}`
- `GET /api/quiz/user-progress?userId={userId}`
